...
if [ "$REPORT_COVERAGE" != "" ]; then
    CMAKE_ARGS+=("-DDD_ENABLE_COVERAGE=ON")
fi
...
# In coverage mode, use the coverage target to generate a report; otherwise just build
if [ "$REPORT_COVERAGE" != "" ]; then
    cmake --build build --target coverage
    stat -t build/coverage/html/index.html
    if [ "${CI:-}" == "" ]; then
