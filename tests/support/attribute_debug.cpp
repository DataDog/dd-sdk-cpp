...
)
{
    switch (a.type)
    {
        ...
        case ValueType::Object:
        {
            ...
            // Accumulate the total size, making sure to include the heap-allocated
            // CowValue struct in our total
            size_t total_size = sizeof(CowValue);
            if (a.type == ValueType::String)
            {
                // For strings, add the number of bytes allocated on the heap by
                // std::string (which may be 0 if using SSO)
            ...
            else if (a.type == ValueType::Array)
            {
                ...
                // std::vector<Attribute>, then recursively compute the heap size of all
                // array items
            ...
            else if (a.type == ValueType::Object)
            {
                ...
                // std::vector<std::pair<std::string,Attribute>>, then add the number of
                // bytes reserved by each property name string, then recursively sum
                // the heap sizes of all property values
                ...
                for (const auto& kvp : cow_ptr->value.object)
                {
                    ...
                    total_size +=
                        AttributeDebug::ComputeHeapSizeImpl(kvp.second, visited);
                }
            }
            return total_size;
        }
