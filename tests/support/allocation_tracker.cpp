...
static void* backend_alloc(size_t count, size_t req_align, bool nothrow)
{
    ...
    // We place a header immediately before the user bytes: the raw block that we get
    // from malloc will encompass: <padding> + <header> + <user bytes (count)>
...
#endif
    // Return our final stats: this value will be 0 (with !enabled) if allocation
    // tracking is not enabled in this build
    return _stats;
}
