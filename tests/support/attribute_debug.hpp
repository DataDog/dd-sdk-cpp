...
{
    /**
     * Generates a string dump that lists all values held within a given Attribute,
     * nested hierarchically. Each primitive value is prefixed with '.' to indicate
     * inline storage; each non-primitive value held via CowValue is prefixed with a
     * number indicating the current reference count of that CowValue.
     */
