 ...
 */
inline std::vector<std::thread> RunParallel(size_t n, std::function<void(size_t)> func)
{
    ...
    // Move ownership of the std::thread vector to the caller: this allows the test to
    // do work in the main thread (concurrent with all threads) before joining
    return threads;
}
...
RunSequential(size_t n, std::function<void(size_t)> func)
{
    for (size_t i = 0; i < n; i++)
    {
        func(i);
