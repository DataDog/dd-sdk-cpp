...
TEST_CASE("dd_attribute", "[unit][attribute][c-api]") {
    ...
    std::vector<TypeTest> types = {
        ...
        {"object",
         ...
         [](const dd_attribute_t* attr) {
           ...
           dd_attribute_t value_foo = dd_attribute_object_property_get(attr, "foo");
           dd_attribute_t value_1 = dd_attribute_object_value_at(attr, 1);
           dd_attribute_t value_bar = dd_attribute_object_property_get(attr, "bar");
           REQUIRE(value_0.type == DD_VALUE_TYPE_INT);
           REQUIRE(value_foo.type == DD_VALUE_TYPE_INT);
  ...
  SECTION("M not leak memory W primitive values are used without being freed") {
    // NOTE: This test documents an implementation detail: it essentially verifies that
    // misuse of the API is technically safe for a certain subset of values. You should
    // always call dd_attribute_free() as required by the API, regardless of the type of
    // the underlying value.

    // Given an array: which we must always free
    dd_attribute_t array = dd_attribute_array(8);

    // And a series of primitive (i.e. non-string, non-compound) values: which should be
    // freed when we're done with them, even though they don't technically hold heap
    // memory, because that's an implementation detail, and because just freeing
    // everything is much more consistent and foolproof as an API design choice
  ...
  */
  ...
  dd_attribute_t obj = dd_attribute_object(3);
  {
    dd_attribute_t process = dd_attribute_object(5);
    {
      ...
      dd_attribute_object_property_set(&process, "guid", &guid);
