...
struct CapturedResourceEvent {
  nlohmann::json obj;

  CapturedResourceEvent(nlohmann::json in_obj) : obj(std::move(in_obj)) {}
};
      ...
      : config(APPLICATION_ID),
        ...
        feature_scope(
            ...
            [this](Block event, Block event_metadata) {
              ...
              // Require that "type" is either "resource" or "error", and that all IDs
              // match our test values
              REQUIRE((obj["type"] == "resource" || obj["type"] == "error"));
              REQUIRE(obj["application"]["id"] == APPLICATION_ID);
              REQUIRE(obj["session"]["id"] == SESSION_ID);
              REQUIRE(obj["view"]["id"] == VIEW_ID);
              if (obj["type"] == "resource") {
                REQUIRE(obj["resource"]["id"] == RESOURCE_ID);
              }
