...
TEST_CASE("ReadCrashContext", "[unit][crash_reporting]") {
  ...
  SECTION("M parse crash context file and return expected field values") {
    ...
    REQUIRE(
        got.rum_initial_config.application_id ==
        *UUID::Parse("a991ca10-4004-4004-4004-beefbeefbeef")
    );
    REQUIRE(got.rum_initial_config.session_sample_rate == 55.5f);
    REQUIRE(
  ...
  SECTION("M parse V1 crash context file and default user_anonymous_id to UUID::Zero") {
    ...
    // And rum_initial_config.session_sample_rate defaults to 0.0f (absent in V1)
    REQUIRE(result.data->rum_initial_config.session_sample_rate == 0.0f);
