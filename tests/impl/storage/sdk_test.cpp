 ...
 * structure: <root>/.datadog/<instance>/<pid>/<feature>/{v1,intermediate-v1}/
 *
 * If `create_lockfile` is true, also creates the lockfile at
 * <root>/.datadog/<instance>/<pid>.lock
 */
void CreateAbandonedDirectory(
  ...
  std::string base = std::string(root) + "/.datadog/" + std::string(instance) + "/" +
                     std::string(pid) + "/" + std::string(feature);
  fs.Mkdirs(base + "/v1");
  fs.Mkdirs(base + "/intermediate-v1");

  if (create_lockfile) {
    std::string lockfile = std::string(root) + "/.datadog/" + std::string(instance) +
                           "/" + std::string(pid) + ".lock";
    fs.Touch(lockfile);
  }
}
...
void WriteTestEvent(
  ...
  std::string path = std::string(root) + "/.datadog/" + std::string(instance) + "/" +
                     std::string(pid) + "/" + std::string(feature) + "/" +
...
TEST_CASE("SdkStorage", "[unit][storage]") {
  ...
  SECTION("M create new directory structure W no abandoned directories exist") {
    ...
    // And the per-process directory is created under the instance directory
    REQUIRE(DirectoryExists(fs, "my-app/storage/.datadog/main/12345"));
  ...
  SECTION("M migrate events from all abandoned directories W multiple exist") {
    ...
    REQUIRE(DirectoryExists(fs, "my-app/storage/.datadog/main/12345"));
    REQUIRE(!DirectoryExists(fs, "my-app/storage/.datadog/main/88888"));
    REQUIRE(!DirectoryExists(fs, "my-app/storage/.datadog/main/99999"));
    REQUIRE(!DirectoryExists(fs, "my-app/storage/.datadog/main/77777"));
  ...
  SECTION("M skip non-numeric directories W scanning for abandoned directories") {
    ...
    REQUIRE(FileExists(fs, "my-app/storage/.datadog/main/not-a-pid.lock"));
    REQUIRE(VerifyEventFile(
        fs,
        "my-app/storage",
        "not-a-pid",
  ...
  SECTION("M migrate events from all features W directory claimed via rename") {
    // Given a single abandoned directory containing multiple features (rum, logs), with
    // events in both v1 and intermediate-v1 subdirectories
    CreateAbandonedDirectory(fs, "my-app/storage", "99999", "main", "rum", true);
  ...
  SECTION("M not orphan abandoned directory W lockfile acquisition fails") {
    ...
    fs.Mkdirs("my-app/storage/.datadog/main");
    fs.Touch("my-app/storage/.datadog/main/12345.lock");

    PlatformPath held_path;
    REQUIRE(held_path.Encode("my-app/storage/.datadog/main/12345.lock"));
    ...
    REQUIRE(DirectoryExists(fs, "my-app/storage/.datadog/main/99999"));
    REQUIRE(FileExists(fs, "my-app/storage/.datadog/main/99999.lock"));
    REQUIRE(!DirectoryExists(fs, "my-app/storage/.datadog/main/12345"));
