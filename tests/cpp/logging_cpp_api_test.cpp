...
TEST_CASE("Logger::Log", "[unit][logging][cpp-api]")
{
    ...
    SECTION("M emit only messages at or above threshold W log threshold is set")
    {
        ...
        const size_t num_warn = std::count_if(
            events.begin(),
            events.end(),
            [](const std::string& s)
            { return s.find("status\":\"warn") != std::string::npos; }
        );
        const size_t num_err = std::count_if(
            events.begin(),
            events.end(),
            [](const std::string& s)
            { return s.find("status\":\"err") != std::string::npos; }
        );
        const size_t num_crit = std::count_if(
            events.begin(),
            events.end(),
            [](const std::string& s)
    ...
    SECTION("M apply sampling rate after log threshold W both are set")
    {
        ...
        const size_t num_info = std::count_if(
            events.begin(),
            events.end(),
            [](const std::string& s)
...
TEST_CASE("Logger thread-safety", "[unit][logging][cpp-api][thread-safety]")
{
    ...
    auto threads = RunParallel(
        20,
        [logging](size_t thread_id)
        {
            const std::string name = "logger-" + std::to_string(thread_id);
            auto logger =
                logging->CreateLogger(LoggerConfig().SetService("svc").SetName(name));
            for (int i = 0; i < 100; ++i)
            {
    ...
    size_t num_events_without_xyz = std::count_if(
        events.begin(),
        events.end(),
        [](const std::string& s) { return s.find(",\"xyz\":") == std::string::npos; }
    );
    REQUIRE(num_events_without_xyz > 0);

    // And some events have a 'xyz' value 'loop-%d'
    size_t num_events_with_valid_xyz = std::count_if(
        events.begin(),
        events.end(),
        [](const std::string& s)
