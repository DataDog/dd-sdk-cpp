...
struct MockFeatureRegistration
{
    std::shared_ptr<T> impl;
    std::unique_ptr<TempDirectory> tempdir;
    std::unique_ptr<MockHttpClient> client;

    std::vector<impl::RegisteredFeature>& vec_ref;
    int32_t vec_i;

    impl::RegisteredFeature& Registered()
    {
        return vec_ref[vec_i];
    }
};

template<typename T = MockFeature>
MockFeatureRegistration<T> RegisterMockFeature()
{}
