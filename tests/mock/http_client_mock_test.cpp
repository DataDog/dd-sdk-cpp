...
TEST_CASE("MockHttpClient Default Behavior", "[unit][mock-http-client]")
{
    ...
    SECTION("M return default HTTP 200 response W no configuration")
    {
        ...
        size_t body_pos = 0;
        auto body_writer = [&test_body, &body_pos](char* buffer, size_t num_bytes) -> size_t {
        ...
        auto result = client.Post("https://api.datadoghq.com/test", "Content-Type: application/json\n", body_writer);
...
TEST_CASE("MockHttpClient Response Simulation", "[unit][mock-http-client]")
{
    ...
    SECTION("M return transient network error W SimulateTransientNetworkError is called")
    {
...
TEST_CASE("MockHttpClient Body Streaming", "[unit][mock-http-client]")
{
    ...
    SECTION("M read body in multiple chunks W body writer returns partial data")
    {
        ...
        std::string full_body = "This is a long message that will be streamed in multiple chunks";
        size_t chunk_size = 10;
        size_t body_pos = 0;

        auto chunked_writer = [&full_body, &body_pos, chunk_size](char* buffer, size_t num_bytes) -> size_t {
    ...
    SECTION("M handle empty body W writer immediately returns EOF")
    {
        ...
        auto empty_writer = [](char*, size_t) -> size_t {
            return HTTP_WRITE_RESULT_EOF;
        };
    ...
    SECTION("M handle aborted request W writer returns ABORT")
    {
        ...
        auto aborting_writer = [&partial_body, &should_abort](char* buffer, size_t) -> size_t {
            if (should_abort) {
