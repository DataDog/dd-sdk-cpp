...
class BatchWriter {
 private:
  ...
  /**
   * Details of the last file we wrote an event to with our current consent value.
   * Reinitialized when we start a new batch or when tracking consent changes.
   *
 ...
 private:
  /**
   * Attempts to write the given event to a batch file within the directory for the
   * current consent value, reusing _active_file if it exists and is still valid for
   * writing to, and creating a new file otherwise.
   */
  bool FlushEvent(Block event, Block event_metadata);

  /**
   * Resolves the appropriate file that we should use to write the next event to,
   * whether that's _active_file or a newly-created file. Repopulates _active_file.
   *
   * Returns a pointer to the StoragePath buffer that holds the full path to a batch
   * file, which may or may not exist. A return value of nullptr indicates that no file
   * is available and the event can not be stored.
   */
  StoragePath* PrepareFileForNextWrite(
