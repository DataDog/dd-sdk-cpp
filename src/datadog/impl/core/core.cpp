  ...
  if (_config.tracking_consent != value) {
    ...
    if (_state == CoreState::Started) {
      ...
      for (auto& feature : _features) {
        if (!feature.event_storage->SetConsent(value)) {
...
bool Core::RegisterFeature(const std::shared_ptr<Feature>& impl) {
  ...
  // Initialize the EventStorage that the storage thread will use to persist events for
  // this feature, creating its per-feature subdirectory and consent subdirectories
  // under the per-PID events root
  auto event_storage = std::make_unique<EventStorage>(
      ...
      *_subsystems.clock,
      EventStorageConfig::FromBatchSize(_config.batch_size)
  );
  if (!event_storage->Initialize(_subsystems.sdk_storage->GetEventsRoot())) {
    _diagnostic_logger.Error(
