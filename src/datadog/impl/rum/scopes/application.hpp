...
class RumApplicationScope {
 ...
 public:
  ...
  /**
   * Returns and clears the last view event generated by any view scope in the active
   * session. Returns std::nullopt if no view event has been generated since the
   * previous call, or if there is no active session.
   */
