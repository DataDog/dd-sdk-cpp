...
class RumApplicationScope {
 ...
 public:
  ...
  /**
   * Returns a struct that describes the essential details of the active session, if one
   * exists, or else the most-recently-active session if no session is active due to an
   * explicit StopSession() call. Returns std::nullopt if RUM has not yet started.
   */
  std::optional<RumSessionState> GetCurrentSessionState() const;

  /**
   * Returns and clears the last view event generated by any active view scope. Returns
   * std::nullopt if no view event has been generated since the previous call, or if
   * there has never been an active session.
   */
  std::optional<RumViewEvent> ConsumeLastActiveViewEvent();
};
