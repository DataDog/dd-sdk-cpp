...
RumFeatureContext RumContext::ToFeatureContext() const {
  ...
  if (session_id == UUID::Zero || !session_is_active || !session_is_sampled) {
    return RumFeatureContext{
        application_id, UUID::Zero, UUID::Zero, UUID::Zero, std::string{}
    };
  }
  ...
  if (active_view_id == UUID::Zero) {
    return RumFeatureContext{
        application_id, session_id, UUID::Zero, UUID::Zero, std::string{}
    };
  }
  ...
  if (active_action_id == UUID::Zero) {
    return RumFeatureContext{
        application_id, session_id, active_view_id, UUID::Zero, active_view_name
    };
  }

  // We have an active action
  return RumFeatureContext{
      application_id, session_id, active_view_id, active_action_id, active_view_name
  };
}
