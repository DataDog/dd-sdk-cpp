    ...
    , _event_object(config.initial_attribute_capacity + 8)
{
    if (config.name)
    {
        _logger_object.attribute.SetObjectProperty(
            "name", Attribute::String(*config.name)
        );
    }
    _logger_object.attribute.SetObjectProperty(
        "version", Attribute::String(SDK_VERSION)
    );
}
...
)
{
    ...
    {
        // Sampling rate defines percentage of messages to _keep_ (e.g.
        // 0.9f => 90% sampled), so discard if our roll is _above_ the threshold
