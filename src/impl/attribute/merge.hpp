...
struct AttributeMerge
{
    ...
    /**
     * Given a target object `mut_obj` and any number of other object attributes,
     * mutates the target object such that it contains the union of all top-level
     * properties found in both `mut_obj` and `attributes`.
     *
     ...
     *
     * If a `filter` function is provided, each property value being merged into
     * `mut_obj` will first have its name checked. If `filter(name)` returns false, the
     * incoming value will be entirely ignored. If `mut_obj` has an existing value with
     * that name, it will be retained. This function does not remove any existing
     * properties from `mut_obj`.
     *
