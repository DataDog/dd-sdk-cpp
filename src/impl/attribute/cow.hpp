...
private:
    /**
     * Initializes a new CowValue of the given type, with refcount 1, preallocating
     * space for the desired number of elements.
     *
...
public:
     ...
     *
     * The caller (i.e. the Datadog Attribute implementation) MUST call Release()
     * whenever a CowValue pointer goes out of scope.
     */
     ...
     * independently modified without affecting references to the original value that
     ...
     *
     * Note that IsShared() and Clone() do not occur atomically: it's assumed that no
     * two threads will attempt to modify the same CowValue concurrently.
     */
    bool IsShared() const;
...
public:
    ...
    /**
     * If storing a string, returns its length. If storing an array, returns the number
     * of items in that array. If storing an object, returns the number of properties
     * in that object.
     */
    size_t Size() const;
     ...
     * index if found. If not storing an object, or if no such property is found,
     * returns -1.
     */
    ...
    /**
     * If storing an object, and if the object has a property with the given name,
     * removes that property from the object. Otherwise, has no effect.
     */
