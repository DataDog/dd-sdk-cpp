...
)
{
    ...
    // there are no conflicts (each property requires ~32 bytes, so overestimating
    // is fine)
    ...
    // Ensure that our target object has enough space to hold all the values we might
    // need to set, preallocating if necessary
    mut_obj.ReserveObjectPropertyCapacity(max_num_properties);

    // Merge in all top-level properties from our input objects, with input objects
    // that appear later in the list taking precedence in case of name conflicts
    for (const Attribute& attribute : attributes)
    {
