    ...
    : type(in_type)
{
    ...
    switch (type)
    {
        case CowValueType::String:
            // NOTE: This branch is technically unreachable, as all production code uses
            // the std::string_view constructor to initialize string values
        ...
        case CowValueType::Object:
            new (&value.object)
                std::vector<std::pair<std::string, datadog::Attribute>>();
            value.object.reserve(initial_capacity);
            break;
    }
}

CowValue::CowValue(std::string_view string_value)
    : type(CowValueType::String)
{
...
CowValue::CowValue(const CowValue& other)
    : ref_count(1)
    , type(other.type)
{
    ...
    // underlying CowValues until the corresponding attributes on the clone are
    // modified.
    switch (type)
    {
        case CowValueType::String:
...
void CowValue::Push(const Attribute& item)
{
    ...
    {
        // Copy item into array, invoking Attribute copy constructor and incrementing
        // item refcount if non-primitive
...
int CowValue::FindPropertyIndex(std::string_view name) const
{
    if (type == CowValueType::Object)
    {
        ...
        if (found != value.object.end())
        {
            ...
            // If we have an object with more than INT_MAX values, something's
            // wrong, but fall out to return -1 in that case rather than truncating
...
void CowValue::SetProperty(std::string_view name, const Attribute& attribute)
{
    if (type == CowValueType::Object)
    {
        ...
        if (found != value.object.end())
        {
            // Copy-assign attribute into vector, handling AddRef()/Release() calls
            // seamlessly (via Attribute copy-assignment operator) for non-primitive
            // values
        ...
        {
            // No existing property has this name: append it, creating a copy of the
            // name as well as the (possibly-refcounted) attribute
