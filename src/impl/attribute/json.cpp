...
static size_t _string_quoted_escaped_write(
    uint8_t* dst, size_t n, std::string_view value
) {
  uint8_t* ptr = dst;
...
static size_t _object_write(uint8_t* dst, size_t n, const impl::CowValue& value) {
  ...
  *ptr++ = '{';
  ...
  for (int i = 0, num = static_cast<int>(num_properties); i < num; i++) {
    ...
    ptr +=
        _string_quoted_escaped_write(ptr, dst_end - ptr, value.GetPropertyNameCStr(i));
    *ptr++ = ':';
    ptr += AttributeSerialization::WriteValue(value.GetAt(i), ptr, dst_end - ptr);
  }
...
void AttributeSerialization::ToJSON(
    const Attribute& attribute, std::vector<uint8_t>& out_buffer
) {
