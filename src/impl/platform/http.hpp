...
enum class HttpResultType : uint8_t
{
     ...
     * implementation (e.g. failed allocation, bad invariants), indicating a fatal
     * error state.
     */
    SentNoRequest,
    /**
    ...
    /**
     * We failed to complete the request due to transient network conditions, but the
...
public:
     ...
     *
     * @param url The fully-qualified URL, including origin. Any query parameters
