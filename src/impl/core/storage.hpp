...
struct BatchWriterConfig
{
     ...
     * maximum of 500 writes, a file will contain no more than 500 TLV Metadata blocks
     * and 500 TLV Event blocks.
     */
...
private:
    ...
    /**
     * Specifies how large a batch file may get, how old it can get before we stop
     * writing to it, etc.
     */
...
public:
     ...
     *  guaranteed: on success, all requested data is written to the file; on failure,
     *  no no data is written.
     */
    bool HandleWrite(Block event, Block event_metadata);
};
