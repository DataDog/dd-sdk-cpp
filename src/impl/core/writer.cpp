...
{
    const size_t n =
        _handle_chunked_write(mut_dst, dst_size, s.data(), s.size(), offset);
    mut_dst += n;
    return n;
}
...
size_t TLVBatchWriter::operator()(char* buffer, size_t num_bytes)
{
    ...
    // We're using our BatchReader to read from a file, one TLV block at a time,
    // into the reader's reusable buffer: this is our "read buffer". The HTTP client
    // provides us with `buffer`, which we will populate with the next chunk of the
    // request body up to `num_bytes` in size: this is our "write buffer".
    ...
    // Keep writing events, separated by the delimiter, until we run out of events or
    // fill the buffer
    while (state.mode == Mode::Event || state.mode == Mode::Delimiter)
    {
        if (state.mode == Mode::Event)
        {
            // Populate state.s with a view of the next event block to write:
            // state.Done() is true when we haven't yet read a block, or when the block
            // we last read has been fully written
            while (state.Done() && !eof)
            {
                ...
                // We only care about event blocks; if we hit a metadata block, skip it
                // and keep reading
                ...
                // We have an event block: accept it into our state struct, making
                // state.Done() no longer true
            ...
            // Write event data if we have it: state.Done() will be false here if we
            // just read a block and reset state.s, -or- if the previous callback
            // invocation only had partial space for the last block and we're resuming
    ...
    if (state.mode == Mode::Suffix)
    {
        ...
        // writing the suffix, the subsequent call to this function will fall down to
        // this point and return 0, signaling EOF to the HTTP client
        return num_bytes_written;
    }
