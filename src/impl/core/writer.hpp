...
{
    /**
     * Finite states that this writer can be in: at any given time, we're working to
     * write one of these things to the destination buffer.
     */
    ...
    struct State
    {
         ...
         */
        explicit State(std::string_view prefix)
            : s(prefix)
        {
        }
         ...
         * `dst`, incrementing both `dst` and `offset` by the number of bytes written;
         * then returns the number of bytes written.
         *
         * If Done() returns true after a call to Write(), no more source data exists
         * and the caller should transition to the next state.
         */
        size_t Write(char*& dst, size_t dst_size);

        /**
         * Returns true if all data in `s` has been written, including the case where
         * `s` is empty and no writes have occurred.
         */
        bool Done() const
        {
