...
public:
    ...
    ~Queue()
    {
        ...
        // processing involves synchronization with threads that the queue itself does
        // not own. The owner of the Queue MUST call Queue::Stop() explicitly, and join
        // on all threads that use it, before the Queue goes out of scope.
    }
    ...
    std::optional<T> Pop()
    {
        ...
        // Queue is not empty: move the first item out of the queue, transferring
        // ownership to this local variable
        T item = std::move(_items.front());
        _items.pop_front();

        // Finally, transfer ownership of the item to the caller. Don't rely on RVO;
        // ensure that an attempt to copy a noncopyable item type here would fail to
        // compile
        return std::move(item);
    }

    /**
     * Stops all queue processing. Subsequent calls to `Push()` will reject the
     * operation and return false, and all pending `Pop()` calls will eventually return
     * with `std::nullopt` once the queue is drained.
     */
    void Stop()
    {
