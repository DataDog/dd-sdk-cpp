...
void dd_attribute_init_array(dd_attribute_t* attribute, size_t initial_capacity)
{
    ...
    if (attribute->type == DD_VALUE_TYPE_ARRAY)
    {
        get_cow_value_for_write(attribute)->Clear(
            clamp_initial_capacity(initial_capacity)
        );
        return;
    }
...
int dd_attribute_object_property_find(const dd_attribute_t* attribute, const char* name)
{
...
)
{
    ...
    datadog::Attribute cpp_value =
        datadog::impl::AttributeConversion::CopyFromC(*value);
