...
)
{
    ...
    // Perform a lightweight copy to initialize a datadog::Attribute from our value,
    // then set it as a global attribute for the logging feature
...
{
    // Abort if no logger or message provided (but still allow an empty-string message
    // to be logged)
    if (!logger || !message)
    {
        return;
...
)
{
    ...
    // If we've been given a valid object attribute, convert it to the equivalent C++
    // type
